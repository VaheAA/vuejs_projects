  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            <label for="wallet" class="block text-sm font-medium text-gray-700"
              >Тикер</label
            >
            <div class="mt-1m- relative rounded-md shadow-md">
              <input
                v-model="ticker"
                @keydown.enter="add"
                @input="findTicker(ticker)"
                type="text"
                name="wallet"
                id="wallet"
            ...
            <div class="flex bg-white shadow-md p-1 rounded-md flex-wrap">
              <span
                v-for="(coin, idx) in matchedCoins.slice(0, 4)"
                :key="idx"
                @click="add"
      ...
      <template v-if="tickers.length">
        ...
        <hr class="w-full border-t border-gray-600 my-4" v-if="ticker.lenght" />
      </template>
...
export default {
  ...
  methods: {
    async getAllCoins() {
      ...
      const getCoins = Object.entries(data.Data);
      getCoins.forEach((coin) => {
        this.coinsList.push(coin[0]);
      });
    },
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        const res = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=e0f60b75ca76c781796e746ca1ea023a1c13841b1b11f31da0a9344ae4d2d518`
        );
        const data = await res.json();
    ...
    findTicker(tickerName) {
      this.matchedCoins = this.coinsList.filter((coin) =>
        coin.includes(tickerName)
      );
    }
  },
  watch: {
